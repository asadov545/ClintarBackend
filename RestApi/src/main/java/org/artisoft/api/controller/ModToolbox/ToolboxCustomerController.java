...
@RestController
@RequestMapping(value = "api/tb/customer")
@CrossOrigin
public class ToolboxCustomerController {
    ...
    public ResponseEntity<Response<List<Customer>>> filterData(@RequestBody HashMap<String, String > filterData){
        ...
        try {
            List<Customer> customerList =  customerDao.filterDataByModId(filterData,2L);
...
//        try {
//            List<ValueLabel<Long, String>> valueLabels = customerDao.filterCustomerList(2L);
...
//    }
    ...
    @RequestMapping(value = "/ac", method = RequestMethod.GET)
    public ResponseEntity<Response<List<ValueLabel<Long, String>>>> getUserListByLabel(@RequestParam("label") String label){
        try {
            List<ValueLabel<Long, String>> valueLabels =  customerDao.filterCustomerListByLabel(label,2L);
            Response<List<ValueLabel<Long, String>>> response = new Response<>();
            HttpStatus httpStatus = (valueLabels == null) ? HttpStatus.NO_CONTENT : HttpStatus.OK;
            return new ResponseEntity<>(response.generateResponse(httpStatus, valueLabels), HttpStatus.OK);
        }catch (Exception e){
